...
@app.route('/cloud_msg/', methods=['HEAD', 'GET', 'POST'])
def cloud_msg():
    if request.method == 'GET':
        return render_template("cloud_message.html")
    
@app.route('/cloud_map/', methods=['HEAD', 'GET', 'POST'])
def cloud_map():
    if request.method == 'GET':
        return render_template("cloud_map.html")
#  插件api
#   登陆管理
...
if __name__ == "__main__":
    app.run(debug=False, host="0.0.0.0", port=1846, threaded=True)